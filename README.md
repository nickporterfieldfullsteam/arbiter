...
Currently: main app v1.19.0, portal v0.7.0.
...
* **v1.19.0 — Dashboard expansion, rep note emails, portal redesign.**
  Dashboard rebuilt with greeting (time-of-day + PM name), quick stats
  (total projects, active, unreviewed, acceptance rate, avg score, overdue
  ETAs), pipeline summary (horizontal stacked bar by status with legend),
  execution lifecycle distribution (horizontal bar chart by stage), and
  four new action widgets: recently updated (last 7 days), aging
  submissions (5+ days stale), projects without owners, and the existing
  rep notes widget. Rep note emails: new `rep_note_added` event type in
  the Edge Function sends PMs an email when a rep adds a note, with the
  note body and green rep badge matching the app's styling. Portal header
  redesigned to match the main app (compact fixed 48px bar, logo + brand
  on left, email + sign-out on right). Realtime auto-refresh via Supabase
  subscriptions with self-change cooldown to prevent panel flicker.
  Migration 016 added `rep_note_added` to the event_type CHECK constraint.
  `notes_last_seen_at` column on projects for unread tracking.
  106 Playwright tests.
...
* **v1.18.0 — Project notes + status board upgrade.** Replaced the single
  ...
  Notes log appears on all three surfaces: Intake inline panel, Kanban modal,
  and Active Projects execution section. Migration 014 created the table with
  RLS (members + reps), seeded existing `decision_notes` as first entries.
  Fixed RLS policies to use `auth.email()` instead of `auth.users` (which
  caused a 403). Notes use `user_metadata.full_name` for author display name.
  Status board upgraded with config-driven pill colors, clickable cards with
  inline detail panels, notes log (read-only, token-gated via
  `get_status_board_notes` RPC), project description and detail fields,
  column progress bars, overdue ETA highlighting, status summary strip, and
  clear-filters link. Migration 015 added `get_status_board_config` and
  `get_status_board_notes` RPCs. `get_status_board_projects` updated to
  return `detail_vals`. Bug fixes: intake notes save (onchange → oninput),
  criteria editor drag-select (drag only from handle), renamed Decision
  Notes → Intake Notes throughout UI. 98 Playwright tests.
...
│                           numbered 001 through 016. Apply via Supabase
...
├── tests/
│   └── tests/
│       ├── e2e/            Playwright tests, 106 currently.
...
| `project_notes` | Threaded activity log per project (added in 014). Each note has project\_id, workspace\_id, author\_name, author\_email, author\_role (`'pm'` / `'rep'` / `'system'`), body, created\_at. Cascades on project delete. RLS: workspace members can read/write; reps can read/write on own projects (by `submitter_email = auth.email()`). Rep insert requires `author_role = 'rep'`. Replaces the legacy `decision_notes` text column (data migrated on creation). |
...
Currently 106 tests, all green. They run against the **real Supabase** (the test
...
* **Project notes use instant-save, not the draft system.** Each note is an INSERT
  into `project_notes` triggered by pressing Enter. Notes are NOT part of the
  status/revisit Save button flow. The old `decision_notes` column still exists in
  the DB and is still loaded by `loadProjects` (for CSV export backward
  compatibility), but the app no longer writes to it.
* **Notes author name uses `user_metadata.full_name`.** This is set in Supabase
  Auth's `raw_user_meta_data` JSONB. If a user doesn't have `full_name` set,
  notes fall back to email, then 'PM'. The status board cleans up email-as-name
  display by splitting on `@` and title-casing. To set a user's display name:
  `update auth.users set raw_user_meta_data = jsonb_set(coalesce(raw_user_meta_data, '{}'), '{full_name}', '"Name"') where email = '...'`.
* **Status board RPCs.** Three SECURITY DEFINER functions with anon EXECUTE:
  `validate_status_board_token(text)`, `get_status_board_projects(text)` (returns
  `detail_vals` as of v1.18.0), `get_status_board_config(text)` (returns
  `detail_fields` as of v1.18.0), `get_status_board_notes(text, uuid)`. All are
  token-gated — they join against `workspace_config.status_board_token`.
...
* Rep-facing notes in the portal — schema and RLS support `author_role = 'rep'`,
  portal UI built. Email notification on rep note added (`rep_note_added` event).
  Dashboard "New rep notes" widget with `notes_last_seen_at` read tracking.
